...
public class NewCallService extends Service {
    ...
    public static final String EXTRA_REMOTE_NICKNAME            = "remote_nickname";
    ...
    class CallBinder extends Binder {
        NewCallService getService() {
            return NewCallService.this;
        }
    }
    ...
    static class ServiceData {
        ...
        NewCallActivity.STATE callState;
        String callId;
        boolean isVideoCall;
        ...
        void set(ServiceData serviceData) {
            ...
            this.remoteNickname = serviceData.remoteNickname;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        mServiceData.remoteNickname = intent.getStringExtra(EXTRA_REMOTE_NICKNAME);
        mServiceData.callState = (NewCallActivity.STATE) intent.getSerializableExtra(EXTRA_CALL_STATE);
    ...
    private static Intent getCallActivityIntent(Context context, ServiceData serviceData, boolean doEnd) {
        ...
        if (serviceData.isVideoCall) {
            intent = new Intent(context, NewVideoCallActivity.class);
        } else {
            intent = new Intent(context, NewVoiceCallActivity.class);
        }
        ...
        intent.putExtra(EXTRA_REMOTE_NICKNAME, serviceData.remoteNickname);
    ...
    public static void dial(Context context, String doDialWithName, String doDialWithCalleeId, boolean isVideoCall) {
        if (SendBirdCall.getOngoingCallCount() > 0) {
            ToastUtils.shortToast("Ringing.");
        ...
        serviceData.remoteNickname = doDialWithName;
        serviceData.callState = NewCallActivity.STATE.STATE_OUTGOING;
        serviceData.callId = doDialWithCalleeId;;
    ...
    public static void onRinging(Context context, @NonNull DirectCall call) {
        ...
        serviceData.remoteNickname =  UserInfoUtils.getNicknameOrUserId(call.getRemoteUser());
        serviceData.callState = NewCallActivity.STATE.STATE_ACCEPTING;
    ...
    private static void startService(Context context, ServiceData serviceData) {
        ...
        if (context != null) {
            Intent intent = new Intent(context, NewCallService.class);
