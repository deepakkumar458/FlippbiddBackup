package com.flippbidd.activity.Cluster;
...
public class CustomeClusterRenderer<T extends ClusterItem> implements ClusterRenderer<T> {
...
//    private static final int[] BUCKETS = {10, 20, 50, 100, 200, 500, 1000};
    ...
    public CustomeClusterRenderer(Context context, GoogleMap map, ClusterManager<T> clusterManager) {
        mMap = map;
        mAnimate = true;
...
//        outline.getPaint().setColor(0x80ff6700); // Transparent white.
...
//        return BUCKETS[BUCKETS.length - 1];
    ...
    /**
     * Transforms the current view (represented by mClusters and mZoom) to a
    ...
    private class RenderTask implements Runnable {
        ...
        @SuppressLint("NewApi")
        public void run() {
            if (clusters.equals(CustomeClusterRenderer.this.mClusters)) {
                mCallback.run();
                return;
            }
            ...
            if (CustomeClusterRenderer.this.mClusters != null && mAnimate) {
                existingClustersOnScreen = new ArrayList<>();
                for (Cluster<T> c : CustomeClusterRenderer.this.mClusters) {
            ...
            mMarkers = newMarkers;
            CustomeClusterRenderer.this.mClusters = clusters;
            mZoom = zoom;
